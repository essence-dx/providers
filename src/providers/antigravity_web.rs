// Google Antigravity (Free) - OAuth authentication
...
#[async_trait]
impl Authenticator for AntigravityWeb {
    ...
    async fn authenticate(&self) -> Result<AuthToken, anyhow::Error> {
        ...
        // Extract from Chrome cookies (uses Google auth like Gemini)
...
#[async_trait]
impl Provider for AntigravityWeb {
    fn id(&self) -> &str {
        "antigravity-web"
    }
    ...
    async fn is_configured(&self) -> bool {
        // Check if we can extract cookies from Chrome (uses Google auth)
        cookie_extractor::extract_gemini_session().is_ok()
    }

    async fn setup(&mut self) -> anyhow::Result<()> {
        println!("  Google Antigravity uses OAuth authentication via Google account.");
        println!("  Please ensure you are logged into a Google account in Opera or Chrome.");
        println!("  Antigravity is free during preview with generous rate limits.");
        println!("  Download the IDE from: https://antigravity.google");
        println!("  Then this provider will automatically extract your session.");

        // Try to authenticate
        match self.authenticate().await {
            Ok(_) => {
                println!("  ✅ Successfully extracted Antigravity session from browser!");
                println!(
                    "  Note: Antigravity provides access to Gemini 3 Pro and Claude Opus 4.5."
                );
                Ok(())
            }
            Err(e) => {
                anyhow::bail!(
                    "Failed to extract Antigravity session: {}. Please log into a Google account in Opera or Chrome first.",
                    e
                )
            }
        }
    ...
    async fn chat(&self, messages: &[ChatMessage]) -> anyhow::Result<ChatResponse> {
        ...
        // Note: The actual Antigravity API is complex and changes frequently
        // This is a simplified implementation that may need updates
        let response_text = format!(
            "Google Antigravity provider is configured but the backend API implementation is pending. Your message was: {}",
            prompt
        );
    ...
    fn get_free_tier_info(&self) -> String {
        "Free during preview - OAuth via Google account - Access to Gemini 3 Pro & Claude Opus 4.5"
            .to_string()
    }
}
